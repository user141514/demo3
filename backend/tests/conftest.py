...
async def async_client(async_engine, db_session) -> AsyncGenerator[AsyncClient, None]:
    ...
    test_ai_service = AIService(api_key="test-key")
...
async def create_workshop(client: AsyncClient, title: str = "测试工作坊") -> dict:
    resp = await client.post("/api/workshops", json={"title": title})
    assert resp.status_code == 201, f"Create workshop failed ({resp.status_code}): {resp.text}"
    return resp.json()


async def join_workshop(client: AsyncClient, workshop_id: int, name: str = "张三", role: str = "senior") -> dict:
    resp = await client.post(f"/api/workshops/{workshop_id}/join", json={"name": name, "role": role})
...
async def workshop_with_participants(async_client: AsyncClient) -> dict:
    """Workshop with 3 participants (senior, middle, junior)."""
    workshop = await create_workshop(async_client)
    w_id = workshop["id"]
    participants = []
    for name, role in [("张总", "senior"), ("李经理", "middle"), ("王专员", "junior")]:
        p = await join_workshop(async_client, w_id, name, role)
...
async def workshop_at_round2(async_client: AsyncClient) -> dict:
    ...
    workshop = await create_workshop(async_client)
    w_id = workshop["id"]
    await join_workshop(async_client, w_id, "测试用户", "senior")
...
def mock_anthropic(monkeypatch):
    """Replace Anthropic client with a fake that returns canned summaries."""
    import anthropic
    ...
    class FakeResponse:
        def __init__(self, text="【AI汇总】测试总结内容：团队达成以下共识..."):
    ...
    monkeypatch.setattr(anthropic, "Anthropic", lambda *a, **kw: FakeClient())
    return FakeClient
